...
Reference implementation for SEM composition estimation under an extreme
few-shot setting.

Main components
- LOIO split over real SEM images
- Patch-level training and image-level prediction
- Simplex-constrained composition head
- Optional GT-based hard mask for diagnostic evaluation
- Optional SIM-pretrained EfficientNet-B0 backbone

The GT-based hard mask is an oracle-support diagnostic option. It should not
be interpreted as a fully blind SEM-only deployment setting.
"""
...
class CFG:
    ...
    # 0 means that the EfficientNet backbone is fully frozen during real-image fine-tuning.
    real_unfreeze_depth: int = 0

    # Hard masking is kept off by default for the public reference code.
    # Use --eval-hardmask when running the oracle-support diagnostic setting.
    apply_gt_hardmask_train: bool = False
    apply_gt_hardmask_eval: bool = False
...
def load_state_dict_cpu(path: str) -> Dict:
    try:
...
def load_items(cfg: CFG) -> List[Tuple[str, np.ndarray, np.ndarray]]:
    if not os.path.exists(cfg.labels_xlsx):
        raise FileNotFoundError(f"Labels file not found: {cfg.labels_xlsx}")
    ...
    for _, row in df.iterrows():
        path = str(row["__imgpath__"])

        y = row[cfg.elements].values.astype(np.float32)
        y = y / (float(y.sum()) + 1e-8)

        has = row[has_cols].values.astype(np.float32)
        has = (has > 0.5).astype(np.float32)

        y = y * has
        y = y / (float(y.sum()) + 1e-8)

        items.append((path, y, has))
...
class PatchDataset(Dataset):
    def __init__(self, items: List[Tuple[str, np.ndarray, np.ndarray]], split: str, cfg: CFG):
        ...
        self.mean = torch.tensor(transforms.mean).view(3, 1, 1)
        self.std = torch.tensor(transforms.std).view(3, 1, 1)
    ...
    def __getitem__(self, idx: int):
        ...
        if self.split == "train":
            ...
            rng = np.random.RandomState(key)
            y0, x0 = random_crop_coords(img.shape[0], img.shape[1], self.cfg.patch_size, rng)
        else:
            split_bias = {"val": 200_000, "test": 300_000}[self.split]
            key = self.cfg.seed + split_bias + img_i * 1000 + patch_i
            y0, x0 = deterministic_crop_coords(img.shape[0], img.shape[1], self.cfg.patch_size, key)
...
def unfreeze_last_stages_efficientnet(model: Model, depth: int) -> None:
...
def load_stage2_backbone(model: Model, backbone_path: str):
    sd = load_state_dict_cpu(backbone_path)
...
def eval_mae_patch_level(model: Model, loader: DataLoader, cfg: CFG) -> float:
    ...
    for x, y, has in loader:
        ...
        if cfg.apply_gt_hardmask_eval:
            pred = hardmask_with_mask(pred, has)

        err = (pred - y).abs().mean(dim=1)
        maes.append(err.detach().cpu().numpy())

    return float(np.concatenate(maes, axis=0).mean())
...
def train_one(
    ...
    opt = optim.AdamW(get_trainable_params(model, cfg), weight_decay=cfg.weight_decay)
    ...
    for epoch in range(cfg.epochs):
        ...
        for x, y, has in train_loader:
            ...
            loss_comp = criterion(pred_for_loss, y)
...
def predict_image_level(
    ...
    patches = []
    for patch_i in range(cfg.patches_per_image_test):
        key = cfg.seed + 900_000 + fold_idx * 10_000 + patch_i
        y0, x0 = deterministic_crop_coords(img.shape[0], img.shape[1], cfg.patch_size, key)
        patches.append(make_patch(img, y0, x0, cfg.patch_size))
    ...
    weights = EfficientNet_B0_Weights.IMAGENET1K_V1
    transforms = weights.transforms()
    mean = torch.tensor(transforms.mean).view(1, 3, 1, 1)
    std = torch.tensor(transforms.std).view(1, 3, 1, 1)
    x = (x - mean) / (std + 1e-12)

    comp_list = []
    for start in range(0, x.shape[0], cfg.test_forward_bs):
        xb = x[start:start + cfg.test_forward_bs].to(cfg.device)
        comp = model(xb)
        comp_list.append(comp.detach().cpu().numpy())

    comp_all = np.concatenate(comp_list, axis=0)
    pred_mean = comp_all.mean(axis=0).astype(np.float32)

    if cfg.apply_gt_hardmask_eval:
        pred_mean = hardmask_numpy(pred_mean[None, :], has_true[None, :])[0]

    err = np.abs(pred_mean - y_true).astype(np.float32)
    img_mae = float(err.mean())
...
def run_loio(cfg: CFG, condition: str):
    ...
    for leave_idx in range(n):
        ...
        out = predict_image_level(model, test_path, y_true, has_true, cfg, fold_idx=leave_idx)
        ...
        fold_rows.append(
            {
                ...
                **{f"pred_{e}": float(pred_mean[i]) for i, e in enumerate(cfg.elements)},
                **{f"has_{e}": float(has_true[i]) for i, e in enumerate(cfg.elements)},
                **{f"err_{e}": float(elem_err[i]) for i, e in enumerate(cfg.elements)},
            }
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="LOIO runner for few-shot SEM composition estimation."
    )
    parser.add_argument(
        "--img-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="./labels.xlsx",
        help="Excel file with composition and element-presence columns.",
    )
    parser.add_argument(
        "--stage2-backbone",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Output directory for LOIO CSV files.",
    )
    parser.add_argument(
        "--elements",
        type=str,
        ...
        help="Element names. Order defines the output composition vector.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of EfficientNet feature blocks to unfreeze from the end.",
    )
    parser.add_argument(
        "--lambda-leak",
        type=float,
        default=0.0,
        help="Weight for absent-element leakage penalty.",
    )
    parser.add_argument(
        "--apply-train-hardmask",
        action="store_true",
        help="Apply GT-based hard mask during the training loss.",
    )
    parser.add_argument(
        "--eval-hardmask",
        action="store_true",
        help=(
            "Apply GT-based element-support hard mask at evaluation. "
            "This is a diagnostic oracle-support setting."
        ),
    )
    parser.add_argument(
        "--skip-sim",
        action="store_true",
        help="Run only the ImageNet/no-SIM baseline.",
    )
    parser.add_argument(
        "--num-workers",
        type=int,
...
def print_run_header(cfg: CFG, run_sim: bool) -> None:
    ...
    print("[INFO] Stage2 backbone:", cfg.stage2_backbone_path)
    print("[INFO] Run SIM condition:", run_sim)
    print()
...
def main() -> None:
    ...
    cfg.apply_gt_hardmask_eval = args.eval_hardmask
    ...
    sim_checkpoint_exists = os.path.exists(cfg.stage2_backbone_path)
    run_sim = (not args.skip_sim) and sim_checkpoint_exists

    print_run_header(cfg, run_sim=run_sim)

    if args.skip_sim:
        print("[INFO] --skip-sim set. Running only the no-SIM condition.")
    elif not sim_checkpoint_exists:
        print(f"[WARN] SIM backbone checkpoint not found: {cfg.stage2_backbone_path}")
        print("[WARN] Skipping SIM condition. No-SIM baseline will still run.")
    print()
    ...
    summary_csv = os.path.join(cfg.out_dir, "loio_summary.csv")
    folds_csv = os.path.join(cfg.out_dir, "loio_folds.csv")

    pd.DataFrame(summaries).to_csv(summary_csv, index=False, encoding="utf-8-sig")
    pd.DataFrame(fold_rows).to_csv(folds_csv, index=False, encoding="utf-8-sig")
    ...
    print("\n" + "=" * 70)
    print("FINAL LOIO MAE SUMMARY")
    print("=" * 70)
    for s in summaries:
        print(
            f"{s['condition']:>5}: "
            f"{s['loio_mean_mae_all']:.6f} "
            f"(std {s['loio_std_mae_all']:.6f})"
        )
    print("=" * 70)
